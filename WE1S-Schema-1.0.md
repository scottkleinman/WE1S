...
* [Manifests Types, the WE1S Superstructure, and the MongoDB Data Model](#types-superstructure-and-mongodb)
...
####collectors (required)
A list containing the names of the WE1S staff who collected the data. Currently, the schema does not specify a format for names.
...
####processes (optional)
A list containing embedded processes or paths to separate process manifests. Both types follow the same schema, described under `Processes`.
