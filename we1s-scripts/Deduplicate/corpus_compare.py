    ...
    for path, filelist in path_filelists:
        ...
        for row in batch_equality_by_sizegroups(filelist):
            ...
            if row[5] in filelist:       ## Files may be duplicated multiple times.
                logger.debug('  {0:<6} {1:30} {2:<6} {3} '.format(' ', os.path.basename(row[4]), 'x', os.path.basename(row[5])))
        ...
        for row in batch_fnamelist_comparer(filelist, args.threshold):
            count_hits += 1
            resultwriter.writerow(row)
            logger.debug('  {0:<6} {1:30} {2} '.format(row[1], os.path.basename(row[4]), os.path.basename(row[5])))
            if (count_hits % 100) == 0:
                csvfile.flush()
